...
import type { CreatePropertyPayload, EditPropertyPayload } from '../owner.types';
...
export default function AddEditPropertyPage() {
  ...
  const [existingGallery, setExistingGallery] = useState<{ id: number; imageUrl: string }[]>([]);
  ...
  const onSubmit = useCallback(
    (values: FormValues) => {
      ...
      if (isEdit) {
        ...
        updateMutation.mutate({ id: propertyId, payload }, { onSuccess: () => navigate('/owner') });
      } else {
        createMutation.mutate(base as CreatePropertyPayload, { onSuccess: () => navigate('/owner') });
      }
    },
    [isEdit, mainImage, galleryFiles, selectedAmenities, deletedImageIds, propertyId, navigate, createMutation, updateMutation],
  );
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="PricePerNight" type="number" {...register('PricePerNight')} placeholder="0" />
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                {errors.GovernorateId && <p className="text-sm text-destructive">{errors.GovernorateId.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="AreaName">Area Name</Label>
                <Input id="AreaName" {...register('AreaName')} placeholder="e.g. Zamalek" />
                {errors.AreaName && <p className="text-sm text-destructive">{errors.AreaName.message}</p>}
              </div>
            </div>
            <div className="space-y-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            {/* Main Image */}
            <div className="space-y-2">
              <Label>Main Image {!isEdit && <span className="text-destructive">*</span>}</Label>
              <div className="flex items-center gap-4">
                {mainImagePreview ? (
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {existingGallery.map((img) => (
                  <div key={img.id} className="relative h-24 w-32 rounded-lg overflow-hidden border">
                    <img src={img.imageUrl} alt="" loading="lazy" width={128} height={96} className="h-full w-full object-cover" />
                    <button
                      type="button"
                ...
                {galleryPreviews.map((url, i) => (
                  <div key={`new-${i}`} className="relative h-24 w-32 rounded-lg overflow-hidden border">
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" disabled={isPending} className="min-w-[140px] font-semibold">
