...
export const getTenantBookingDetails = (id: number) =>
  api
    .get<BookingApiResponse<TenantBookingDetails>>(`/tenant/${id}`)
    .then((r) => r.data);

// ── Get Booking Details — Owner (GET /api/Booking/owner/{id}) ──
export const getOwnerBookingDetails = (id: number) =>
  api
    .get<BookingApiResponse<OwnerBookingDetails>>(`/owner/${id}`)
    .then((r) => r.data);
...
export const getIncomingBookings = (params: BookingListParams) =>
  api
    .get<BookingApiResponse<PaginatedData<Booking>>>("/incoming-bookings", {
      params,
    })
    .then((r) => r.data.data);
...
export const updateBookingStatus = (
  ...
  api
    .patch<BookingApiResponse<string>>(`/${id}/status`, data)
    .then((r) => r.data);
