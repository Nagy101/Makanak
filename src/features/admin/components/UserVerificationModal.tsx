...
import { X, CheckCircle2, XCircle, Ban, User, Mail, Phone, Calendar, Shield } from 'lucide-react';
...
const UserVerificationModal = memo<UserVerificationModalProps>(({ userId, onClose }) => {
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        {isLoading ? (
          ...
          <p className="py-8 text-center text-muted-foreground">No data found</p>
        ) : (
          <div className="space-y-5 py-2">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <Badge variant="outline" className={statusBadge[data.userStatus]}>
                    {data.userStatus}
                  </Badge>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 text-sm">
                ...
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Calendar className="h-4 w-4" /> Joined {new Date(data.joinAt).toLocaleDateString()}
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Shield className="h-4 w-4" /> {data.userType} · Strikes: {data.strikeCount}
                </div>
              </div>
            </div>
            ...
            {/* National ID */}
            <div className="space-y-3">
              <h4 className="text-sm font-semibold text-foreground">National ID Documents</h4>
              {data.nationalId && (
                <p className="text-sm text-muted-foreground">ID: {data.nationalId}</p>
              )}
