    ...
    <CardContent className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div>
          ...
          <p className="font-medium text-foreground mt-0.5">{data.totalDays}</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className="font-semibold text-primary mt-0.5">
            {data.totalPrice.toLocaleString()} EGP
          </p>
        </div>
        <div>
          <p className="text-muted-foreground text-xs">Your Payout</p>
          <p className="font-semibold text-foreground mt-0.5">
            {data.amountToPayToOwner.toLocaleString()} EGP
          </p>
        </div>
      </div>
    </CardContent>
...
export default function OwnerQRScannerPage() {
  ...
  const scanResult = scanMutation.data?.isSuccess ? scanMutation.data.data : null;
    ...
    try {
      ...
      setCameraError(
        err?.message?.includes('NotAllowedError') || err?.message?.includes('Permission')
          ? 'Camera access denied. Please allow camera permissions or use the image upload option below.'
          : 'Camera not available. Use the image upload option instead.',
      );
    }
  ...
  const handleFileUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        setCameraError('Could not read QR code from image. Please try a clearer photo.');
      }
      // Reset file input
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-muted-foreground">
        Scan a tenant's QR code to confirm their check-in.
      </p>
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
            ...
            {!cameraActive && !scanResult && (
              <div className="flex flex-col items-center justify-center gap-4 py-16 px-4 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Open camera or upload a QR image
                </p>
              </div>
            )}
          </div>
          ...
          {scanMutation.isError && (
            <div className="flex items-start gap-2 rounded-lg bg-destructive/10 px-4 py-3 text-sm text-destructive">
              <AlertCircle className="h-4 w-4 shrink-0 mt-0.5" />
              <span>QR verification failed. Please try again.</span>
            </div>
          )}
