...
const RegisterPageExample = () => {
  ...
  const { governorates, loading: loadingGovernorates, error: govError } = useGovernorates();
    ...
    <div className="space-y-2">
      ...
      >
        <SelectTrigger className="h-11" disabled={loadingGovernorates}>
          <SelectValue placeholder={loadingGovernorates ? "Loading..." : "Select Governorate"} />
        </SelectTrigger>
        <SelectContent>
      ...
      {govError && <p className="text-sm text-destructive">{govError}</p>}
    </div>
  );
};
